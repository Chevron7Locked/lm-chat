...
describe("Chat — stale pinned-model auto-switch", () => {
  ...
  it("does NOT fire for an empty/implicit model_id (backend-resolved default)", () => {
    ...
    // Falls through to savedDefaultModel for DISPLAY only (this is the
    // pre-existing modelId display chain in Chat.tsx, unrelated to the
    // auto-switch effect) — no PATCH/toast, since the implicit-default path
    // must be left untouched regardless of the default's loaded state.
    expect(shownModel()).toBe("lmstudio::idled-default-7b");
    expect(updateChatCalls).toHaveLength(0);
    expect(toastPushCalls).toHaveLength(0);
  });
});
