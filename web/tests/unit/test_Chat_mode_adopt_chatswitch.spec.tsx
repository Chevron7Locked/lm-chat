 ...
 * the chat it actually belongs to.
 *
...
// Controllable SSE mock — `mockSSEState` carries a `modeAdopt` verdict
// tagged with the chatId of the stream that produced it (chat 1's, always,
// in this file), fixed at mount and never reassigned. Any effect refire
// this file observes is caused purely by the chat-navigation identity
// change, not by the verdict itself changing.
...
describe("Chat — mode_adopt (C3) cross-chat scoping", () => {
  ...
  it("applies a mode_adopt verdict to the chat whose stream produced it", () => {
    mockSSEState = {
      ...idleSSEState,
      modeAdopt: { presetId: "coder", msgId: 1, chatId: 1 },
    };
    renderChat("/chats/1");
  ...
  it("does NOT reapply a chat 1 verdict once the user has navigated to chat 2", () => {
    ...
    // sseState is a single shared instance — the verdict object is
    // deliberately NOT reset by this navigate (see the doc above), so it's
    // still sitting there, tagged chatId: 1, once the user is on chat 2.
    navigateTo("/chats/2");
  ...
  it("keeps re-applying (harmlessly) to chat 1 itself across unrelated re-renders, but never to chat 2 or beyond", () => {
    mockSSEState = {
      ...idleSSEState,
      modeAdopt: { presetId: "researcher", msgId: 7, chatId: 1 },
    };
    renderChat("/chats/1");
